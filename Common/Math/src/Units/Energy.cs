...
namespace MathEx.ExternalBallistic.Units
{
    public class Energy
    {
        ...
        static private Energy mConvertor = new Energy(0, Unit.FootPounds);
        ...
        protected void Set(double value, Unit unit)
        {
            ...
            switch (unit)
            {
            case    Unit.FootPounds:
                    mValue = value;
                    break;
            case    Unit.Joule:
                    mValue = value * 0.737562149277;
                    break;
            }
        }
        ...
        public Energy ToUnit(Unit unit)
        {
            return new Energy(Get(unit), unit);
        }
        ...
        static public bool TryParse(string text, out Energy value, out Unit unit)
        {
            return TryParse(text, CultureInfo.CurrentCulture, out value, out unit);
        }

        static public bool TryParse(string text, CultureInfo culture, out Energy value, out Unit unit)
        {
            value = null;
            unit = DefaultUnit;
